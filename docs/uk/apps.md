...
Для точнішого вимірювання можна відкалібрувати температуру полем **Компенсація** (offset) цього ж застосунку у веб-інтерфейсі або ключем `temp_offset` у [dev.json](./dev).
...
Застосунок Battery відображає поточний рівень заряду вбудованого акумулятора з **динамічною вертикальною піктограмою батареї**: вона заповнюється знизу вгору пропорційно до заряду та змінює колір за рівнем (зелений ≥75%, жовтий 50–74%, помаранчевий 20–49%, червоний <20%).
...
| **Air Quality (AQI)** | Індекс якості повітря (US EPA, 1–6). Автоколір за рівнем (зелений → жовтий → помаранчевий → червоний) або фіксований колір. Коли перемикач автокольору увімкнено, колір рівня має пріоритет і перекриває будь-який фіксований колір чи колір елемента (нічний режим усе одно діє). Може також деталізувати забруднювачі, що вийшли за межі норми (див. нижче). |
| **UV Index** | Ультрафіолетовий індекс (0–11+). Автоколір за рівнем або фіксований колір. Коли перемикач автокольору увімкнено, колір рівня має пріоритет і перекриває будь-який фіксований колір чи колір елемента (нічний режим усе одно діє). |
...
### Деталізація забруднювачів (Air Quality)

Коли увімкнено опцію **Show components** (увімкнено за замовчуванням), після показу `ICA:N` застосунок по черзі відображає окремі забруднювачі, які самі по собі **вийшли за межі здорового діапазону** (рівень EU EAQI ≥ 4): `PM2.5`, `PM10`, `O3`, `NO2`, `SO2` та `CO`. Вони показуються **від найгіршого**, не більше **3**, і кожен малюється кольором **власного** рівня. Спрацьовування залежить від значення кожного забруднювача незалежно від загального індексу — тож забруднювач може з'явитися червоним, навіть коли загальний індекс помірний (напр., `ICA:2`, але `O3` червоний).

Час показу кожного забруднювача налаштовується через **Seconds per component** (2–10 с, за замовчуванням 3). Уся послідовність (індекс + позначені забруднювачі) відтворюється в межах однієї появи застосунку; його час на екрані автоматично подовжується, щоб усе вмістилося.

Ці опції розташовані в **Apps → AirQuality** (розгорніть елемент у списку ротації): **Auto color**, **Show components** та поле секунд.

::: tip
Якщо оновлення погоди не вдалося (напр., нестабільний інтернет), на екрані
залишається останнє зчитане значення замість `--`, і повтор відбувається
автоматично приблизно через 1 хвилину. Індикатор WiFi (верхній лівий кут)
блимає червоним, поки завантаження не вдаються.
:::

---
## Місяць

Застосунок «Місяць» показує поточну **фазу Місяця** за допомогою одного з **8 дизайнерських растрових іконок фаз (набір LaMetric), по одній на кожну фазу Місяця** (новий → повний → спадний), намальованого на тлі мерехтливих синіх зірок. Кути диска прозорі, тож зірки видно навколо нього, а в південній півкулі іконка віддзеркалюється.

Поряд із Місяцем по черзі відображаються до трьох налаштовуваних показників:

- **Назва фази** (показується іспанською, напр. "Cuarto creciente", "Llena")
- **Вік Місяця** у днях
- **Відсоток освітленості**

Налаштовується в **Apps → Moon** (розгорніть елемент у ротації):

| Параметр | Опис |
|----------|------|
| **Півкуля** | Північна або Південна — віддзеркалює освітлений край Місяця. |
| **Показувати назву фази / вік / %** | Вмикає або вимикає кожен показник тексту. |

Якщо вимкнути всі три показники, відображається **лише Місяць** — по центру екрана, а сині зірки заповнюють усю ширину.

**Тривалість** і **колір тексту** задаються для кожного елемента ротації, як і в інших застосунках. Фаза обчислюється астрономічно (на основі UTC), тож інтернет не потрібен.
