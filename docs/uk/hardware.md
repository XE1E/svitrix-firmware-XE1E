# Обладнання (Ulanzi TC001)

Апаратна довідка годинника Ulanzi TC001 — платформи, на якій працює SVITRIX.
...
Ulanzi TC001 поставляється з датчиком **SHT3x** (адреса I2C `0x44`), який вимірює температуру та вологість. Показники температури можуть бути завищені через внутрішнє тепло від LED матриці, ESP32 та батареї в герметичному корпусі. Для компенсації налаштуйте зміщення через веб-інтерфейс (**Apps → Температура → Компенсація**) або ключем `temp_offset` у `dev.json`.
