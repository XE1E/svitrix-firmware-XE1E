...
## Зміст
  ...
  * [Ротація (Застосунки + Ефекти)](#ротація-застосунки--ефекти)
  * [Будильники та нагадування](#будильники-та-нагадування)
  * [Погода](#погода)
  ...
  * [Оновлення](#оновлення)
      ...
      - [Налаштування WiFi](#налаштування-wifi)
      - [Резервне копіювання та відновлення налаштувань](#резервне-копіювання-та-відновлення-налаштувань)
  * [Система та файли](#система-та-файли)
      - [Версія прошивки](#версія-прошивки)
      - [Застосування конфігурації мережі/брокера](#застосування-конфігурації-мережіброкера)
      - [Файловий менеджер (LittleFS)](#файловий-менеджер-littlefs)
...
| `N/A`                           | `http://[IP]/api/apps`             | Список застосунків з даними іконок            |

> **Примітка:** MQTT також транслює інші дані, наприклад натискання кнопок та поточний застосунок.

> **Примітка:** `GET /api/apps` повертає застосунки з їхніми іконками. `POST /api/apps` (тіло JSON) оновлює вектор застосунків; також доступно через MQTT на `[PREFIX]/apps`.
...
Відтворення звуків у стилі R2-D2, згенерованих із рядка опису звуку:

| MQTT-топік       | HTTP URL                    | Тіло запиту     | HTTP-метод  |
| ---------------- | --------------------------- | --------------- | ----------- |
| `[PREFIX]/r2d2`  | `http://[IP]/api/r2d2`      | рядок R2D2      | POST        |
...
## Ротація (Застосунки + Ефекти)

Єдина конфігурація ротації: порядок застосунків і ефектів разом із тривалістю та кольором кожного елемента. Замінює старі endpoint'и `/api/reorder` та `/api/playlist`.

| HTTP URL                       | Тіло запиту  | HTTP-метод  | Опис |
| ------------------------------ | ------------ | ----------- | ---- |
| `http://[IP]/api/rotation`     | -            | GET         | Отримати поточну конфігурацію ротації |
| `http://[IP]/api/rotation`     | JSON         | POST        | Зберегти конфігурацію ротації |

> **Примітка:** Лише HTTP, без MQTT-топіків.

Тіло та відповідь використовують ключ `items` з масивом елементів ротації (застосунки та ефекти, кожен із необов'язковими тривалістю та кольором).
...
Керування будильниками та нагадуваннями. Працює без WiFi завдяки RTC.

| MQTT-топік              | HTTP URL                     | Тіло запиту  | HTTP-метод  | Опис |
| ----------------------- | ---------------------------- | ------------ | ----------- | ---- |
| `N/A`                   | `http://[IP]/api/alarms`     | -            | GET         | Список усіх будильників і стан спрацювання |
| `[PREFIX]/alarm/add`    | `http://[IP]/api/alarms`     | JSON         | POST        | Додати новий будильник (або дію snooze/dismiss) |
| `N/A`                   | `http://[IP]/api/alarms`     | JSON         | PUT         | Оновити наявний будильник (за `id`) |
| `N/A`                   | `http://[IP]/api/alarms?id=X`| -            | DELETE      | Видалити будильник із заданим `id` |
| `[PREFIX]/alarm/snooze` | `http://[IP]/api/alarms`     | `{"action":"snooze","minutes":N}` | POST | Відкласти будильник, що дзвонить |
| `[PREFIX]/alarm/dismiss`| `http://[IP]/api/alarms`     | `{"action":"dismiss"}` | POST | Вимкнути будильник, що дзвонить |

### Властивості JSON будильника

| Ключ | Тип | Опис | За замовчуванням |
| --- | ---- | ----------- | ---------------- |
| `id` | integer | Ідентифікатор будильника (лише для PUT). | 0 |
| `hour` | integer | Година (0–23). | 0 |
| `minute` | integer | Хвилина (0–59). | 0 |
| `days` | integer | Бітова маска активних днів тижня. `0x7F` = щодня. | `0x7F` (127) |
| `enabled` | boolean | Чи будильник активний. | true |
| `oneTime` | boolean | Одноразовий будильник (вимикається після спрацювання). | false |
| `snoozeMinutes` | integer | Хвилини відкладання (snooze). | 5 |
| `label` | string | Необов'язкова текстова мітка. | `""` |
| `melody` | string | Звук для відтворення: ім'я файлу RTTTL з папки MELODIES або повний RTTTL-рядок. | `""` |

> **Примітка:** Для snooze через MQTT на `[PREFIX]/alarm/snooze` можна надіслати `{"minutes":N}`; якщо пропущено, використовується `snoozeMinutes` будильника. Відповідь GET також містить поле `ringing` (boolean), яке вказує, чи зараз дзвонить будильник.
...
Налаштування та отримання даних зовнішньої погоди. Джерелом може бути WeatherAPI.com (через `apiKey`) АБО ваш власний сервер (через `serverUrl`). Якщо `serverUrl` не порожній, годинник опитує цей URL замість WeatherAPI, і він має повертати формат `current.json` від WeatherAPI (наприклад, `https://clima.xe1e.net/api/svitrix`) з необов'язковими додатковими полями `solar_radiation`, `precip_event_mm`, `rain_rate_mm`. Погодні застосунки (зовнішня температура, вологість, тиск, якість повітря, UV, вітер, радіація, опади) додаються та впорядковуються на вкладці «Застосунки» (ротація).

| HTTP URL                          | Тіло запиту  | HTTP-метод  | Опис |
| --------------------------------- | ------------ | ----------- | ---- |
| `http://[IP]/api/weather`         | -            | GET         | Отримати конфігурацію погоди |
| `http://[IP]/api/weather`         | JSON         | POST        | Оновити конфігурацію погоди |
| `http://[IP]/api/weather/fetch`   | -            | POST        | Примусово оновити дані негайно |
| `http://[IP]/api/weather/data`    | -            | GET         | Отримати поточні дані погоди |

> **Примітка:** Лише HTTP, без MQTT-топіків.

### JSON конфігурації погоди

| Ключ | Тип | Опис |
| --- | ---- | ----------- |
| `apiKey` | string | API-ключ постачальника погоди. |
| `serverUrl` | string | Власний сервер. Якщо не порожній, годинник опитує цей URL замість WeatherAPI. Має повертати формат `current.json` від WeatherAPI (+ необов'язкові додаткові поля `solar_radiation`, `precip_event_mm`, `rain_rate_mm`). Порожній = використовувати WeatherAPI.com. |
| `locationType` | integer | Тип локації (0 = місто, 1 = координати, 2 = ID станції). |
| `city` | string | Назва міста. |
| `latitude` | float | Широта. |
| `longitude` | float | Довгота. |
| `stationId` | string | ID метеостанції. |
| `updateInterval` | integer | Інтервал оновлення у хвилинах (1–120; варіанти в UI 1/2/5/10/15/30/60). З власним сервером тримайте його низьким (1–2), щоб відстежувати швидкозмінні значення. |
| `showOutdoorTemp` | boolean | Показувати застосунок зовнішньої температури. |
| `showOutdoorHumidity` | boolean | Показувати застосунок зовнішньої вологості. |
| `showPressure` | boolean | Показувати застосунок тиску. |
| `showAirQuality` | boolean | Показувати застосунок якості повітря (AQI). |
| `showIndoorTemp` | boolean | Показувати застосунок внутрішньої температури. |
| `showIndoorHumidity` | boolean | Показувати застосунок внутрішньої вологості. |
| `showUV` | boolean | Показувати застосунок UV-індексу. |
| `outdoorTempColor` | integer | Колір зовнішньої температури (ціле RGB). |
| `outdoorHumColor` | integer | Колір зовнішньої вологості (ціле RGB). |
| `pressureColor` | integer | Колір тиску (ціле RGB). |
| `aqiColor` | integer | Колір AQI (ціле RGB). |
| `uvColor` | integer | Колір UV-індексу (ціле RGB). |
| `aqiAutoColor` | boolean | Автоматичний колір AQI за рівнем. |
| `aqiShowComponents` | boolean | Після `ICA:N` по черзі показувати забруднювачі (PM2.5, O3, NO2…), що перевищують рівень 4, кожен у кольорі свого рівня. Увімкнено за замовчуванням. |
| `aqiComponentSecs` | integer | Скільки секунд показується кожен забруднювач у ротації (2–10, за замовчуванням 3). |
| `uvAutoColor` | boolean | Автоматичний колір UV-індексу за рівнем. |
| `outdoorTempDuration` | integer | Тривалість застосунку зовнішньої температури (секунди). |
| `outdoorHumDuration` | integer | Тривалість застосунку зовнішньої вологості (секунди). |
| `pressureDuration` | integer | Тривалість застосунку тиску (секунди). |
| `aqiDuration` | integer | Тривалість застосунку AQI (секунди). |
| `uvDuration` | integer | Тривалість застосунку UV-індексу (секунди). |
| `showWind` | boolean | Показувати застосунок вітру. |
| `windColor` | integer | Колір вітру (ціле RGB). |
| `windDuration` | integer | Тривалість застосунку вітру (секунди). |
| `windShowGust` | boolean | Додавати порив ` R<n>` до рядка вітру, напр. `W 5 R4`. |
| `showRadiation` | boolean | Показувати застосунок сонячної радіації (Вт/м²). |
| `radColor` | integer | Колір радіації (ціле RGB). |
| `radAutoColor` | boolean | Автоматичний колір радіації за інтенсивністю (як UV). |
| `radDuration` | integer | Тривалість застосунку радіації (секунди). |
| `showPrecip` | boolean | Показувати застосунок опадів. |
| `precipColor` | integer | Колір опадів (ціле RGB). |
| `precipDuration` | integer | Тривалість застосунку опадів (секунди). |
| `precipShowRate` | boolean | Додавати інтенсивність дощу ` <n>/h` до рядка, напр. `0.0mm 0.0/h`. |

`GET /api/weather/data` повертає поточні дані: `valid`, `outdoorTemp`, `outdoorHumidity`, `pressure`, `aqi`, `uv`, `condition`, `conditionCode`, `windSpeed` (км/год), `windDeg`, `windGust` (км/год), `windDir`, `solarRadiation` (Вт/м²), `precipToday` (мм), `precipEvent` (мм поточної події), `rainRate` (мм/год), `lastUpdate` (мілісекунди), `failStreak` (кількість послідовних невдалих мережевих запитів; 0 = останній успішний).

> **Самовідновлення:** якщо запит застрягає, годинник спершу **повторно під'єднується до WiFi** (м'яке відновлення, після кількох поспіль невдач) і лише якщо все одно провів max(15 хв, 5× інтервал) без успішного запиту, з підключеним WiFi, він **перезавантажується** як останній засіб (скидає мережевий стек/пам'ять). Він тримає WiFi без енергозбереження (годинник живиться від USB), щоб з'єднання не «застаріло». У звичайній роботі він ніколи не перезавантажується; `failStreak` та `/api/nvs` дозволяють стежити за його станом.

> **Нічний режим:** поки активний нічний режим, годинник **призупиняє всі запити** (погода + власні джерела) — показується лише годинник — і заморожує таймер самовідновлення (він не перезавантажується вночі). Коли нічне вікно завершується, він **повторно під'єднується до WiFi** і одразу виконує запит, тож застосунки погоди знову з'являються зі свіжими даними.
...
| `TEFF`        | number                    | Вибір ефекту переходу між застосунками (повний список у `/api/transitions`).                         | 0–13                                               | 1          |
...
| `GAMMA`       | number                    | Гамма-корекція дисплея. `1.9` = нейтрально; вище = більше контрасту / темніші середні тони; нижче = пласкіше. | Додатне число з рухомою комою                  | 1.9    |
...
Перегляд або зміна мереж WiFi (до 3) через API. `eraseWifi` очищує лише облікові дані WiFi і перезавантажує (вужче, ніж `erase`).

| HTTP URL                       | Тіло запиту                                                        | HTTP-метод  |
|--------------------------------|-------------------------------------------------------------------|-------------|
| `http://[IP]/api/wifi`         | -                                                                 | GET         |
| `http://[IP]/api/wifi`         | `{"networks":[{"ssid":"МояМережа","password":"..."}]}`            | POST        |
| `http://[IP]/api/eraseWifi`    | -                                                                 | POST        |

#### Резервне копіювання та відновлення налаштувань

Експортуйте всі налаштування як JSON або відновіть їх із резервної копії. Див. також [Резервне копіювання та відновлення](./backup).

| HTTP URL                            | Тіло запиту               | HTTP-метод  |
|-------------------------------------|---------------------------|-------------|
| `http://[IP]/api/settings/export`   | -                         | GET         |
| `http://[IP]/api/settings/import`   | JSON (попередня копія)    | POST        |

## Система та файли

Системні endpoint'и та прямий доступ до файлової системи LittleFS (іконки, мелодії, користувацькі застосунки, резервні копії).
...
#### Діагностика NVS (розділ налаштувань)

Використання розділу NVS (20 КБ), де зберігаються всі налаштування. Якщо він заповнюється, NVS псується і налаштування втрачаються (годинник переходить у режим точки доступу AP). Стежте, щоб `usedPct` залишався нижче ~85%.

| HTTP URL               | Тіло запиту  | HTTP-метод  | Опис                                                       |
|------------------------|--------------|-------------|------------------------------------------------------------|
| `http://[IP]/api/nvs`  | -            | GET         | `{used, free, total, namespaces, usedPct}` (записи по 32 байти) |

#### Застосування конфігурації мережі/брокера

Застосовує файл `/DoNotTouch.json` (статичний IP, MQTT-брокер, Home Assistant, NTP, авторизація) до системи "на льоту", без перезавантаження. Використовується SPA після зміни розширених налаштувань.

| HTTP URL           | Тіло запиту  | HTTP-метод  |
|--------------------|--------------|-------------|
| `http://[IP]/save` | form-data    | POST        |

#### Файловий менеджер (LittleFS)

Перегляд, завантаження, вивантаження та видалення файлів у сховищі (`/ICONS/`, `/MELODIES/`, `/CUSTOMAPPS/`, `/PALETTES/`). Це те, що використовує сторінка «Файли» в SPA.

| HTTP URL                            | Тіло запиту               | HTTP-метод  | Опис                                          |
|-------------------------------------|---------------------------|-------------|-----------------------------------------------|
| `http://[IP]/list?dir=/ICONS`       | -                         | GET         | Список каталогу (JSON: ім'я, тип, розмір)     |
| `http://[IP]/<шлях>`                | -                         | GET         | Завантажити файл (напр. `/ICONS/962.jpg`)     |
| `http://[IP]/edit?filename=/<шлях>` | multipart (поле `data`)   | POST        | Вивантажити або створити файл                 |
| `http://[IP]/edit`                  | `path=/<шлях>`            | PUT         | Створити каталог                              |
| `http://[IP]/edit`                  | `path=/<шлях>`            | DELETE      | Видалити файл                                 |

> **Примітка:** відповіді з файлами можуть бути стиснені gzip (`Content-Encoding: gzip`); використовуйте клієнт, який розпаковує автоматично.
