...
Налаштування та отримання даних зовнішньої погоди (потрібен API-ключ). Погодні застосунки з'являються в ротації відповідно до перемикачів `showOutdoorTemp`, `showOutdoorHumidity`, `showPressure`, `showAirQuality` та `showUV`.
...
| `updateInterval` | integer | Інтервал оновлення у хвилинах. |
...
`GET /api/weather/data` повертає поточні дані: `valid`, `outdoorTemp`, `outdoorHumidity`, `pressure`, `aqi`, `uv`, `condition`, `conditionCode`, `lastUpdate`.
