...
> **Самовідновлення:** якщо годинник провів max(15 хв, 5× інтервал) без успішного запиту, а WiFi усе ще підключений, він перезавантажує сам себе, щоб скинути мережевий стек; у звичайній роботі він ніколи не перезавантажується; `failStreak` та `/api/nvs` дозволяють стежити за його станом.

> **Нічний режим:** поки активний нічний режим, годинник **призупиняє всі запити** (погода + власні джерела) — показується лише годинник — і заморожує таймер самовідновлення (він не перезавантажується вночі). Коли нічне вікно завершується, відкладений запит виконується негайно, тож застосунки погоди знову з'являються зі свіжими даними.
